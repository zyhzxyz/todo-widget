...
- 右键任务开始计时，浮窗和主窗口底部均有暂停、继续、停止按钮；浮窗跟随主题。
- 顶部垃圾桶/右下角“回收站”分别保留已完成、已删除任务，支持撤销完成、按日期撤销目标打卡和恢复删除；完成/删除后也有可见撤销提示。
- 搜索、日历、设置、日记互斥，重复点击或 Escape 关闭；窄窗口工具栏自动换行。
...
当前同步是近实时轮询：本机修改立即提交，其他端修改约每 5 秒刷新，重新聚焦/联网也刷新；不是 WebSocket，运行中的秒表停止/完成后才上传计时。

计时、面板、回收站的[独立测试验收步骤](docs/UI_ACCEPTANCE.md)包含新版配套升级说明；先测试再迁移原始记录。恢复删除会保留完成状态和历史，但提醒需重新设置。旧版本已经物理删除且没有备份的任务无法恢复。
