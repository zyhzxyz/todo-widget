...
> 当前主线为本地存储版。服务器与 AstrBot 联动会在独立功能分支开发；历史 `*_OPTIMIZATION.md` 等文件是开发过程记录，不代表当前验收结果。近期修复见 [CHANGELOG.md](CHANGELOG.md)。
...
- 旧数据中的每日目标按起止日期和每天完成记录显示；当前普通创建面板不提供每日目标入口。
...
不要通过 Syncthing 双向同步正在使用中的 LevelDB 目录。也不要将个人数据或访问令牌提交到 Git。升级前应保留备份；后续服务器迁移会提供显式 JSON 导出/导入，而不是直接搬运浏览器数据库文件。
